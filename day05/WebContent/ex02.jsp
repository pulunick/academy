...
<fieldset>
	...
	<h3>주의, EL Tag의 본질은 출력이다. 연산은 자바에서 끝내는 것이 좋다. EL Tag는 연산을 최소화해야한다</h3>
	<h3>\${'10' + '20' } : ${'10' + '20' }</h3>
	<h3>\${"10" + "23" } : ${"10" + "23" }</h3>
	<h3><%-- ${'Hello' + 'world' } --%></h3>
	<h3>${'Hello' }${'world' }</h3>
	<br><br>
	<h3>삼항연산 가능하다.</h3>
	<h3>${arr[0].name }은 ${arr[0].age >= 35 ? '성인' : '미성년자' }</h3>
	<h3>${arr[1].name }은 ${arr[1].age >= 35 ? '성인' : '미성년자' }</h3>
	<h3>${arr[2].name }은 ${arr[2].age >= 35 ? '성인' : '미성년자' }</h3>
	<h3>${arr[3].name }은 ${arr[3].age >= 35 ? '성인' : '미성년자' }</h3>
	<h3>${arr[4].name }은 ${arr[4].age >= 35 ? '성인' : '미성년자' }</h3>
</fieldset>
</body>
</html>