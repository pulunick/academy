...
<%
	...
	// 이름과 같이 1:1일 수도 있지만, request.getParameterValues(); 와 같이 값이 여러개일 수도 있다
...
<%
	...
	// 개발자가 만들어서 넣는 모든 유형의 자바 객체는 attribute로 처리한다
	// 내장객체.setAttribute(String name, Object o);
	request.setAttribute("isPasswordCorrect", isPasswordCorrect);
	// 상속에서 다형성의 개념 (업캐스팅) => 즉 값을 반환할때 강제다운캐스팅을 해줘야하는 불편함이 존재한다.
%>
<!-- 파라미터는 사용자가 만들어내는 값이다. -->
<!-- Attribute는 개발자가 반환하는 것이다. 즉, 모든 자료형을 반환을 할 수 있다. -->
<h3>비밀번호가 일치하는가 : <%=request.getAttribute("isPasswordCorrect") %></h3>
<!--  EL 표현식 -->
<!-- Attribute의 문법을 간소화하기 위해 등장한 문법--><!-- 기본jsp의 내장라이브러리로 저장되어있다 -->
